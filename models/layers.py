"""Common layers for defining score networks.(Done)
"""
import math
import string
...
def get_act(config):
  """Get activation functions from the config file. 从配置文件中获取激活函数名称"""
...
def variance_scaling(scale, mode, distribution,
  ...
  def init(shape, dtype=dtype):
    ...
    if distribution == "normal":
      return jt.randn(*shape, dtype=dtype) * np.sqrt(variance)
    elif distribution == "uniform":
      return (jt.rand(*shape, dtype=dtype) * 2. - 1.) * np.sqrt(3 * variance)
    else:
...
class MSFBlock(nn.Module):
  ...
  def forward(self, xs, shape):
    sums = jt.zeros(xs[0].shape[0], self.features, *shape, device=xs[0].device)
    for i in range(len(self.convs)):
      h = self.convs[i](xs[i])
      h = nn.interpolate(h, size=shape, mode='bilinear', align_corners=True)
      sums += h
    return sums
...
class UpsampleConv(nn.Module):
  ...
  def forward(self, inputs):
    output = inputs
    output = jt.cat([output, output, output, output], dim=1)
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
  ...
  emb = jt.cat([jt.sin(emb), jt.cos(emb)], dim=1)
  if embedding_dim % 2 == 1:  # zero pad
    emb = nn.pad(emb, (0, 1), mode='constant')
...
def _einsum(a, b, c, x, y):
  ...
  return jt.einsum(einsum_str, x, y)
...
class NIN(nn.Module):
  def __init__(self, in_dim, num_units, init_scale=0.1):
    ...
    self.b = nn.Parameter(jt.zeros(num_units), requires_grad=True)
...
class AttnBlock(nn.Module):
  """Channel-wise self-attention block.(已检查)"""
  ...
  def forward(self, x):
    ...
    w = jt.linalg.einsum('bchw,bcij->bhwij', q, k) * (int(C) ** (-0.5))
    w = jt.reshape(w, (B, H, W, H * W))
    w = nn.softmax(w, dim=-1)
    w = jt.reshape(w, (B, H, W, H, W))
    h = jt.linalg.einsum('bhwij,bcij->bchw', w, v)
    h = self.NIN_3(h)
    return x + h
...
class Upsample(nn.Module):
  ...
  def forward(self, x):
    B, C, H, W = x.shape
    h = nn.interpolate(x, (H * 2, W * 2), mode='nearest')
    if self.with_conv:
      h = self.Conv_0(h)
    return h
...
class Downsample(nn.Module):
  ...
  def forward(self, x):
    ...
    if self.with_conv:
      ...
      x = nn.avg_pool2d(x, kernel_size=2, stride=2, padding=0)
...
class ResnetBlockDDPM(nn.Module):
  """The ResNet Blocks used in DDPM.(已检查)"""
