...
def get_pc_inpainter(sde, predictor, corrector, inverse_scaler, snr,
  ...
  def get_inpaint_update_fn(update_fn):
    ...
    def inpaint_update_fn(model, data, mask, x, t):
      with jt.no_grad():
        vec_t = jt.ones(data.shape[0]) * t
        x, x_mean = update_fn(x, vec_t, model=model)
        masked_data_mean, std = sde.marginal_prob(data, vec_t)
        masked_data = masked_data_mean + jt.randn_like(x) * std[:, None, None, None]
...
def get_pc_colorizer(sde, predictor, corrector, inverse_scaler,
  ...
  M = jt.Var([[5.7735014e-01, -8.1649649e-01, 4.7008697e-08],
  ...
  invM = jt.linalg.inv(M)

  # Decouple a gray-scale image with `M`
  def decouple(inputs):
    return jt.einsum('bihw,ij->bjhw', inputs, M)

  # The inverse function to `decouple`.
  def couple(inputs):
    return jt.linalg.einsum('bihw,ij->bjhw', inputs, invM)
  ...
  def get_mask(image):
    mask = jt.cat([jt.ones_like(image[:, :1, ...]),
                      jt.zeros_like(image[:, 1:, ...])], dim=1)
    return mask
